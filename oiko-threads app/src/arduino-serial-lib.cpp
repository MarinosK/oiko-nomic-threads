...
//
int arduino_lib::serialport_close( int fd )
{
  return close( fd );
}

//
int arduino_lib::serialport_writebyte( int fd, uint8_t b)
{
  int n = write(fd,&b,1);
  if( n!=1)
    return -1;
...
//
int arduino_lib::serialport_write(int fd, const char* str)
{
  int len = strlen(str);
...
//
int arduino_lib::serialport_read_until(int fd, char* buf, char until, int buf_max, int timeout)
{
...
//
int arduino_lib::serialport_flush(int fd)
{
